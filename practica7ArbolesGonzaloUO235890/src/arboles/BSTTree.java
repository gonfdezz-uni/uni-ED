...
public class BSTTree<T extends Comparable<T>> implements Tree<T> {
	...
	private TreeNode<T> add(TreeNode<T> currentNode, T element) {
		if (currentNode == null) {
	...
	private TreeNode<T> removeRecursive(TreeNode<T> currentNode, T elementoAEliminar) {
		if (currentNode == null) {
		...
		if (comparacion == 0) {
			...
			if (currentNode.right == null) {
				...
				TreeNode<T> mayorDeMenores = getMaxNode(currentNode);
				// 2. Susutituir la clave del nodo a borrar por la clave del mayor de menores
				mayorDeMenores.element = elementoAEliminar;
				// 3. Borrar el nodo cuya clave es ese máximo
				remove(mayorDeMenores.element);
				// 4. Devolver subárbol modificado
			}
		} else if (comparacion < 0) {
			currentNode.right = removeRecursive(currentNode.left, elementoAEliminar);
		} else {
			currentNode.left = removeRecursive(currentNode.right, elementoAEliminar);
		}
		return currentNode;
	}
